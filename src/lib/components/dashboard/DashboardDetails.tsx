import React from "react";
import DashboardDetailCard from "./DashboardDetailCard";
import { PiMoneyLight } from "react-icons/pi";
import { CiClock2 } from "react-icons/ci";
import { BsArchive } from "react-icons/bs";
import { BsPeople } from "react-icons/bs";
import { queryDashboardDetails } from "@/lib/utils/supabase/serverQueries";

async function DashboardDetails() {
  const {
    collectedAndPendingData,
    amountOfInvoicesData,
    amountofCustomersData,
    collectedAndPendingError,
    amountOfInvoicesError,
    amountofCustomersError,
  } = await queryDashboardDetails();

  return (
    <div>
      <h1 className="text-2xl font-semibold mb-3">Dashboard</h1>
      <div className="flex flex-row max-lg:flex-wrap grow gap-x-5 gap-y-5">
        {collectedAndPendingData && (
          <DashboardDetailCard
            icon={<PiMoneyLight className="mt-1" />}
            title="Collected"
            data={`$${collectedAndPendingData[0].sum.toString()}`}
          />
        )}

        {collectedAndPendingData && (
          <DashboardDetailCard
            icon={<CiClock2 className="mt-1" />}
            title="Pending"
            data={`$${collectedAndPendingData[1].sum.toString()}`}
          />
        )}

        {amountOfInvoicesData && (
          <DashboardDetailCard
            icon={<BsArchive className="mt-1" />}
            title="Total invoices"
            data={amountOfInvoicesData[0].count.toString()}
          />
        )}

        {amountofCustomersData && (
          <DashboardDetailCard
            icon={<BsPeople className="mt-1" />}
            title="Total Customers"
            data={amountofCustomersData[0].count.toString()}
          />
        )}
      </div>
    </div>
  );
}

export default DashboardDetails;
